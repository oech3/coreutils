...
impl MultiWriter {
    ...
    pub fn copy_unbuffered(&mut self, mut input: NamedReader) -> Result<()> {
            ...
            ($pipe:expr, $writer:expr, $len:expr) => {
                ...
                if splice_exact($pipe, &fd, $len).is_err() {
                    ...
                    if let Err(e) = ($pipe)
                        ...
                        .and_then(|_| $writer.inner.write_all(&drain))
                    {
        ...
        #[cfg(any(target_os = "linux", target_os = "android"))]
        {
            let (pipe_read, pipe_write) = pipe::<false>(MAX_ROOTLESS_PIPE_SIZE)?; // needed to duplicate input
            let (pipe2_read, pipe2_write) = pipe::<false>(MAX_ROOTLESS_PIPE_SIZE)?; // force-tee() even output is not pipe
